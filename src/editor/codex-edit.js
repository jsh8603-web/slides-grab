...
export function buildCodexEditPrompt({ slideFile, slidePath, userPrompt, selections = [] }) {
  ...
  return [
    ...
    '',
    'NanoBanana Image Regeneration (use when user requests image change/replace/improve):',
    '- Interpret the user\'s intent into a detailed English image prompt following NanoBanana rules:',
    '  - Descriptive sentence (not keyword lists), e.g. "A professional photorealistic..."',
    '  - Always include "No text." at the end',
    '  - Include camera angle, lighting, color hex codes from slide palette, style keywords',
    '  - For non-16:9: prepend [1:1] or other ratio hint',
    '  - Never use "transparent" — use "pure white (#FFFFFF) background"',
    '- Generate image using the single-image mode:',
    '  ```bash',
    '  node scripts/generate-images.mjs --prompt "YOUR_ENGLISH_PROMPT" --output-file "SLIDE_ASSETS_DIR/new-image.png" --aspect-ratio "16:9"',
    '  ```',
    '- After generating, update <img src="..."> in the slide HTML to reference the new file.',
