...
1. NotebookLM 자동 수집 (주제만 입력하면 NLM이 웹에서 자동 조사)
2. NotebookLM 노트북 링크 제공 (이미 만든 노트북이 있을 때)
3. URL 또는 파일 직접 제공 (웹페이지, PDF, 문서 등)
4. AI가 알아서 조사 (일반적인 주제일 때)
```

- 1번 선택 → `node scripts/nlm-auto-research.js --topic "주제"` 실행 (난이도에 따라 Fast/Deep Research 자동 전환, `.claude/rules/notebooklm-fetch.md` 참조)
- 2번 선택 → `.claude/rules/notebooklm-fetch.md` 절차로 내용 추출
- 3번 선택 → WebFetch/Read로 내용 수집
- 4번 선택 → WebSearch로 조사 후 진행
...
이 기록은 다음 프레젠테이션의 Step 2(HTML 생성)에서 동일 이슈를 사전 예방하기 위함이다.
...
2. **전체 슬라이드 프리뷰 확인** — `ppt_get_slide_preview`로 모든 슬라이드를 확인하며 다음을 검사:
   ...
   - **텍스트 누락** (카드/박스 내부 텍스트가 비어있는지)

3. **이슈 발견 시 자동 수정**
   - `ppt_close_presentation` → HTML 수정 → 재변환 → 다시 열어 재검사 (최대 2회)
