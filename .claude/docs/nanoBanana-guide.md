...
## IV/IP/IC/VQA 에러 발생 시 오탐/정탐 분기

| 탐지 | 오탐 시 수정 (탐지 코드) | 정탐 시 수정 (생성 코드) |
|:----:|----------------------|----------------------|
| **IV** | `generate-images.mjs` IV부 | 이 파일의 프롬프트 규칙 |
| **IP** | `generate-images.mjs` IP부 | 이 파일 + `enhancePrompt()` |
| **IC** | IC 프롬프트/임계값 | `plan-skill/SKILL.md` |
| **VQA** | `scoreImageWithVQA` | 이 파일 + `enhancePrompt()` |

기록 의무: 오탐/정탐 무관하게 `pptx-inspection-log.md`에 기록.
전체 프로세스: `CLAUDE.md` 참조. 상세: `vqa-pipeline-maintenance.md` §0.
