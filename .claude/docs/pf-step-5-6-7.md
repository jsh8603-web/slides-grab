...
3. **VP/COM/IC ERROR 또는 오탐 WARN → 오탐/정탐 판정 + 코드 수정 (필수)**
   VP/COM에서 ERROR 또는 오탐 WARN이 발견되면:
   - **오탐/정탐 판정** 후:
     - 오탐 → `validate-pptx.js` 또는 COM 비교 로직 수정
     - 정탐 → HTML 원본 수정 + `design-skill/SKILL.md` 또는 `html-prevention-rules.md` 생성 규칙 추가
   - 코드 수정 시 즉시 `progress.md` 검증 항목에 기재 (§E)
   - 재변환 → VP/COM 재검증 (최대 2회)
   - **ERROR는 반드시 수정. WARN 중 오탐 판정한 것도 탐지코드 수정 필수. 나머지 WARN은 맥락 판단**
...
**게이트 통과 시 필수 행동**:
1. `progress.md`에 "Step 6-3 COM 비교 검증 통과" 기록
